...
### 📋 How to Run (The Bulletproof Method)

**⚠️ Important:** This project requires **Anaconda** or **Miniconda** to run correctly, especially on Mac (M1/M2/M3) to avoid segmentation faults.
...
visual_archive_bot/
│
├── app.py                   # (Standalone/Alternative Streamlit version)
├── archive_builder.py       # (Step 2: Scans images & builds the FAISS index)
├── chatbot_base.py          # (Parent class for the chatbot)
├── download_images.py       # (Step 1: Downloads sample art dataset)
├── README.md                # (Documentation)
├── requirements.txt         # (List of libraries)
├── run_chatbot.py           # (Step 3: The Modular Streamlit app - Preferred for OOP)
├── visual_archive_bot.py    # (The logic class inheriting from ChatbotBase)
│
├── images/                  # (Generated by download_images.py)
│   ├── art_sample_1.jpg
│   └── ...
│
└── index_db/                # (Generated by archive_builder.py)
    ├── image_vectors.index  # The FAISS vector database
    └── metadata.json        # The text data (paths, tags, IDs)
```
